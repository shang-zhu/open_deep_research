...
from libs.utils.evals import Result, ScoringFunction, llm_as_a_judge_scoring
...
def authenticate_huggingface():
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to authenticate with Hugging Face Hub: {str(e)}")
...
def process_single_question(
    ...
    print(f"Running question {idx+1}/{total}")

    agent = create_agent(agent_config)

    agent_answer = agent(goal=question)
...
def main():
    ...
    parser.add_argument(
        "--agent-config",
        default="../configs/base_llm_config.yaml",
        help="Agent config to use (default: ../configs/base_llm_config.yaml)",
    )
    parser.add_argument(
        "--max-workers",
        type=int,
